...
func Logging() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		// This middleware only records business requests, such as the "/hash/xiuxiu/v3" adn "/login"
		req := regexp.MustCompile("(/hash/xiuxiu/v3|/login )")
		if !req.MatchString(path) {
			return
		}

		// Skip for the headlth check sequests.
